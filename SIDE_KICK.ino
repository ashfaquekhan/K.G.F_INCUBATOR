...
int sizmenu[6] ={40,40,99,99,9,1};
char *mainmenu[] ={"TEMP START","TEMP STOP","HUMD START","HUMD STOP","FAN-INTERVAL","RESET"};
int siz;
int opval[6];
bool state=0;
bool hstate=0;
void setup() {
  readIntArrayFromEEPROM(20,opval,6);
  Wire.begin();
...
void pulse(){
  ...
  digitalWrite(EXHAUST, relayState);
}
  ...
  if (myButton.isReleased()) {
    ...
    if(mainmenu[counter]=="RESET"){
      ...
      while(1){
        ...
        if (myButton.isReleased()) { 
            opval[counter]=cn;
            writeIntArrayIntoEEPROM(20,opval,6);
            lcd.clear();
            currentTime = millis();
            goto A;
         }
